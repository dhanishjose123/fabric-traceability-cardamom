...
function createOrgs() {
  ...
  . organizations/fabric-ca/registerEnrolldynamic.sh

  # Wait until any one CA container is ready — using the first org in ORG_NAMES
  FIRST_ORG="${ORG_NAMES[0]}"
  echo "⏳ Waiting for TLS certificate from ${FIRST_ORG} CA to be generated..."
  while :
  do

    if [ ! -f "organizations/fabric-ca/${FIRST_ORG}/tls-cert.pem" ]; then
      sleep 1
    else
      break
    fi
  ...
  # Make sure Fabric CA server is ready (e.g., ca-farmers)
  export FABRIC_CA_CLIENT_HOME=${PWD}/organizations/peerOrganizations/${FIRST_ORG}.example.com/
  COUNTER=0
  rc=1
  PORT=${ORG_CA_PORT_MAP[$FIRST_ORG]}
  while [[ $rc -ne 0 && $COUNTER -lt $MAX_RETRY ]]; do
    sleep 1
    set -x
    fabric-ca-client getcainfo -u https://admin:adminpw@localhost:${PORT} --caname ca-${FIRST_ORG} --tls.certfiles "${PWD}/organizations/fabric-ca/${FIRST_ORG}/ca-cert.pem"
    res=$?
    { set +x; } 2>/dev/null
    rc=$res
    COUNTER=$((COUNTER + 1))
  ...
  # Dynamically create org identities
  for ORG in "${ORG_NAMES[@]}"; do
    PORT=${ORG_CA_PORT_MAP[$ORG]}
    infoln "🔐 Creating identities for $ORG on port $PORT"
    createOrg "$ORG" "$PORT"
  done

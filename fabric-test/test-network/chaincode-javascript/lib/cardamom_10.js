...
async _transfer(ctx, from, to, amount) {
  ...
  const walletKey = `retailers-${retailerId}-wallet`;  // uses same pattern as createWallet
  ...
  const existingOffers = lot.offers || [];
  const maxOfferPerKg = existingOffers.reduce((max, o) => Math.max(max, o.offerPrice), 0);
  ...
  lot.offers.push({
    ...
    // mark lot as under consideration
  await ctx.stub.putState(lotKey, Buffer.from(JSON.stringify(lot)));

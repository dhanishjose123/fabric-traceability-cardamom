...
async consolidateWallet(ctx, org, userId) {
    ...
    const walletKey = ctx.stub.createCompositeKey('wallet', [org, userId]);
    ...
    wallet.lastUpdated = new Date().toISOString();
     // 🔥 Counter logic
    wallet.consolidationCount = (wallet.consolidationCount || 0) + 1;
...
async depositMoney(ctx, org, userId, amount) {
    ...
    // 🔹 Read wallet (needed for consolidationCount)
    const walletKey = `${org}-${userId}-wallet`;
    const walletBytes = await ctx.stub.getState(walletKey);
    ...
    // 🔥 Use dynamic objectType
    const objectType = `walletTx_${currentCount}`;

    const txKey = ctx.stub.createCompositeKey(objectType, [
        org,
        userId,
        txId
    ]);
    ...
    return `✅ Deposited ₹${amountValue} to ${org}-${userId} (tx: ${txId})`;
}


async getWalletBalance(ctx, org, userId) {

    // 🔹 1. Read wallet snapshot
    const walletKey = `${org}-${userId}-wallet`;
    const walletBytes = await ctx.stub.getState(walletKey);
    ...
    let baseBalance = wallet.balance || 0;

    // 🔥 USE CONSOLIDATION COUNT
    const currentCount = wallet.consolidationCount || 0;

    // 🔹 2. Read ONLY current batch
    const objectType = `walletTx_${currentCount}`;
...
async _transfer(ctx, from, to, amount) {
    ...
    // 🔹 Split org and user
    const [fromOrg, fromUser] = from.split('.');
    const [toOrg, toUser]     = to.split('.');

    // 🔹 Read wallet
    const fromWalletKey = `${fromOrg}-${fromUser}-wallet`;
    const toWalletKey   = `${toOrg}-${toUser}-wallet`;
    ...
    // 🔹 Create keys
    const fromTxKey = ctx.stub.createCompositeKey(fromObjectType, [
        fromOrg,
        fromUser,
        txId,
        'debit'
    ...
    const toTxKey = ctx.stub.createCompositeKey(toObjectType, [
        toOrg,
        toUser,
        txId,
        'credit'
    ...
    console.log(`✅ Transferred ₹${parsedAmount} from ${from} to ${to} (txId: ${txId})`);

    return `Transferred ₹${parsedAmount} from ${from} to ${to}`;
}
  ...
  const txTimestamp = ctx.stub.getTxTimestamp();
  const testedAt = new Date(txTimestamp.seconds.low * 1000).toISOString();

  // Basic test info
  lot.status = result === 'pass' ? 'APPROVED' : 'REJECTED';
  lot.testResult = result;
  lot.testedAt = testedAt;
 ...
 async makeOffer1(ctx, lotId, retailerId, offerPrice) {
    this._logInvocation("makeOffer", arguments, ctx);
    console.log("🚀 Function `makeOffer` invoked");

    this._requireOrg(ctx, 'RetailersMSP');

    // 🔹 1. Fetch lot (READ ONLY)
    const lotKey = ctx.stub.createCompositeKey('lot', [lotId]);
    const lotBytes = await ctx.stub.getState(lotKey);
    ...
    if (lot.status !== 'APPROVED' ) {
        throw new Error('Offers allowed only for APPROVED or ongoing lots');
    }
    ...
    // 🔥 3. Get highest offer ON-CHAIN (helper function)
    const highestOffer = await this._getHighestOfferForLot(ctx, lotId);

    if (highestOffer && newPricePerKg <= highestOffer.offerPrice) {
        throw new Error(
            `Offer must be higher than current highest offer ₹${highestOffer.offerPrice}/kg`
        );
    }
    ...
    // 🔥 6. Append-only composite key (CRITICAL FIX)
    const txId = ctx.stub.getTxID();
    const offerKey = ctx.stub.createCompositeKey('offer', [lotId, retailerId, txId]);
    ...
    // 🔹 7. Store offer ONLY (no lot update → MVCC safe)
    await ctx.stub.putState(offerKey, Buffer.from(JSON.stringify(newOffer)));
...
async _getHighestOfferForLot(ctx, lotId) {
    ...
    while (true) {
        ...
        if (res.value && res.value.value.toString()) {
            ...
            if (!highestOffer || offer.offerPrice > highestOffer.offerPrice) {
                highestOffer = offer;
            }
        }
    }
    ...
    return highestOffer; // 🔥 returns object, not string
}
...
async getHighestOfferForLot(ctx, lotId) {
  ...
  const txTimestamp = ctx.stub.getTxTimestamp();
  const soldAt = new Date(txTimestamp.seconds.low * 1000).toISOString();
...
async reconstructPacketLifecycle(ctx, packetId) {
    ...
    while (true) {
        ...
        if (res.value) {
            ...
            /* ---------------------------
               🔹 Packing
            --------------------------- */
            if ((record.event === "PACKED" || record.packedAt) && !lifecycle.packed) {
                lifecycle.packed = {
                    txId,
                    timestamp,
                    packedBy: record.owner
                };
            }
            ...
            /* ---------------------------
               🔹 Ownership transfer
            --------------------------- */
            if (prevOwner && record.owner !== prevOwner) {
                lifecycle.ownerTransfers.push({
                    txId,
                    from: prevOwner,
                    to: record.owner,
                    timestamp
...
async reconstructLotLifecycle(ctx, lotId) {

    const lotKey = ctx.stub.createCompositeKey('lot', [lotId]);
    const iterator = await ctx.stub.getHistoryForKey(lotKey);
    ...
    while (true) {
        ...
        if (res.value) {
            ...
            // SUBMITTED
            if (record.status === "SUBMITTED" && !lifecycle.submitted) {
                lifecycle.submitted = { owner: record.owner, timestamp };
            }

            // TESTED
            if (record.testResult && !lifecycle.tested) {
                lifecycle.tested = { owner: record.owner, timestamp };
            }

            // SOLD
            if (record.status === "SOLD" && !lifecycle.sold) {
                lifecycle.sold = { owner: record.owner, timestamp };
            }

            // OWNERSHIP
            lifecycle.ownership.push({
                owner: record.owner,
                status: record.status,
                timestamp
            });
    ...
    lifecycle.ownership.reverse();   // ✔ FIX ORDER

    return lifecycle;
}

async getPacketLifecycle(ctx, packetId) {

    const packetLifecycle = await this.reconstructPacketLifecycle(ctx, packetId);

    if (!packetLifecycle) {
        throw new Error('No lifecycle found');
    }

    /* 🔥 Extract lotId separately */
    const packetKey = ctx.stub.createCompositeKey('packet', [packetId]);
    const iterator = await ctx.stub.getHistoryForKey(packetKey);
    ...
    const lotLifecycle = await this.reconstructLotLifecycle(ctx, lotId);
...
async getProduceByOwner(ctx, ownerId, limitStr) {
  ...
  console.log("🚀 Function `getProduceByOwner` invoked");

  const iterator = await ctx.stub.getStateByPartialCompositeKey('lot', []);
  const results = [];

  let count = 0;
  const maxRecords = limitStr ? parseInt(limitStr) : Number.MAX_SAFE_INTEGER;
...
async getLotsWithOffersByOwner(ctx, ownerId, limit) {
    ...
    while (true) {
        ...
        // 🔥 Only include if at least one offer exists
        if (hasOffer) {

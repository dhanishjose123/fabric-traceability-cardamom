...
async getApprovedLotsWithoutOffers(ctx, limitStr) {
    this._logInvocation("getLotsWithNoOffers", arguments, ctx);
    ...
    while (true) {
        ...
        if (!hasOffer) {
            ...
            // 🔥 LIMIT BREAK (critical for performance)
            if (count >= limit) {
                break;
            }
        }
